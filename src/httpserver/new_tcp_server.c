#if PLATFORM_BEKEN_NEW || NEW_TCP_SERVER
	...
	while(true)
	{
		...
		if(new_idx < max_socks)
		{
			...
			else
			{
				...
				{
					ADDLOG_DEBUG(LOG_FEATURE_HTTP, "[sock=%d]: TCP Client thread creation failed!", sock[new_idx].fd);
...
error:
	ADDLOG_DEBUG(LOG_FEATURE_HTTP, "TCP Server Error");
