...
static int ReadFlashVars(void* target, int dataLen)
{
	...
	int readLen;
	ADDLOG_DEBUG(LOG_FEATURE_CFG, "%s: will read %d bytes", __func__, dataLen);
	readLen = ef_get_env_blob(KV_KEY_FLASH_VARS, target, dataLen, NULL);
	ADDLOG_DEBUG(LOG_FEATURE_CFG, "%s: really loaded %d bytes", __func__, readLen);
	g_loaded = 1;
	return dataLen;
}
...
static int SaveFlashVars(void* src, int dataLen)
{
	...
	{
		ADDLOG_DEBUG(LOG_FEATURE_CFG, "%s: EF_ENV_INIT_FAILED for %d bytes", __func__, dataLen);
		return 0;
	}
	if(res == EF_ENV_NAME_ERR)
	{
		ADDLOG_DEBUG(LOG_FEATURE_CFG, "%s: EF_ENV_ARG_ERR for %d bytes", __func__, dataLen);
		return 0;
	}
	ADDLOG_DEBUG(LOG_FEATURE_CFG, "%s: saved %d bytes", __func__, dataLen);
	return dataLen;
}
